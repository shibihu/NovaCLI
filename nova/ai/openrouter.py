...
Uses OpenRouter's OpenAI-compatible API endpoint. All failure modes are converted
...
MISSING_KEY_HINT = """No OpenRouter API key configured.

Set OPENROUTER_API_KEY in environment or credentials file (~/.nova/credentials.json).
...
class OpenRouterProvider:
    ...
    def __repr__(self) -> str:
        return (
            f"OpenRouterProvider(model={self._model!r}, "
    ...
    async def complete(
        ...
        """Return the assistant response from OpenRouter."""
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/shibihu/NovaCLI",
            "X-Title": "NovaCLI",
    ...
    def _extract_response(self, data: dict[str, Any]) -> AIResponse:
        ...
        if not choices:
            raise AIProviderError("OpenRouter returned an empty response (no choices).")
        ...
        if raw_tool_calls:
            for idx, tc in enumerate(raw_tool_calls):
                call_id = tc.get("id") or f"call_openrouter_{idx}"
