    ...
    async def complete(
        ...
        try:
            ...
            if resp.status_code in (401, 403):
                raise MissingAPIKeyError(f"Cerebras rejected the API key (HTTP {resp.status_code}). {MISSING_KEY_HINT}")
            if resp.status_code != 200:
                error_body = self._sanitize(resp.text[:300])
                raise AIProviderError(f"Cerebras returned HTTP {resp.status_code}: {error_body}")
            ...
            raise AIProviderError(f"Could not reach Cerebras ({type(exc).__name__}): {self._sanitize(str(exc))}") from exc
        except httpx.TimeoutException as exc:
            raise AIProviderError(f"Cerebras request timed out after {self._timeout:.0f}s.") from exc
        except (httpx.HTTPError, json.JSONDecodeError) as exc:
            raise AIProviderError(f"Cerebras request failed: {self._sanitize(str(exc))}") from exc
